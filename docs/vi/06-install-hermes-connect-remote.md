...
Đảm bảo terminal đang ở Bash (mặc định) và thư mục làm việc là `/workspaces/codespaces-hermes-server`.

```bash
pwd
# Kết quả: /workspaces/codespaces-hermes-server
```
...
Kéo file `.zip` từ máy local thả vào **Explorer panel** trong VS Code (trên trình duyệt) để upload lên Codespace. File sẽ xuất hiện trong `/workspaces/codespaces-hermes-server/`.
...
> 1. Kiểm tra log chi tiết: `cat /workspaces/codespaces-hermes-server/hermes.log`
